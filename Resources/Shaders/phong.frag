...
uniform vec3 camSpace_lightData[8]; //If point, refers to position, if directional, refers to direction
...
vec4 getToLight(int lightIndex) {
    ...
    if (lightType[lightIndex] == LIGHT_POINT) {
        return vec4(normalize(camSpace_lightData[lightIndex] - vec3(camSpace_pos)), 0);
    }
    else if (lightType[lightIndex] == LIGHT_DIRECTIONAL) {
        return vec4(normalize(-camSpace_lightData[lightIndex]), 0);
    }
...
float attenuationFactor(int lightIndex) {
    ...
    if (lightType[lightIndex] == LIGHT_POINT) {
        ...
        float d = length(camSpace_lightData[lightIndex] - vec3(camSpace_pos));
...
void main() {
    ...
    if (texUsed) {
        ...
        fragColor = vec4(tempColor*color, 1.0);
    }
    else {
